...
self.onmessage = (event, handle) => {
  var msg = event.data;

  if (msg.type != 'create-actor') return;

  if (!msg.body) {
    postMessage({ error: 'Missing message body in "create-actor" message' });

    return;
  }

  if (!msg.body.parent) {
    postMessage({ error: 'Missing parent definition in "create-actor" message' });
  ...
  if (!def) {
    postMessage({ error: 'Missing actor definition in "create-actor" message' });
  ...
  if (msg.body.additionalRequires) {
    try {
      ...
      postMessage({ error: 'Error while requiring additional modules: ' + err });
  ...
  system.rootActor()
    ...
    .catch(err => {
      ...
      try {
        postMessage({ error: errMsgPrefix + ' ' + err });
      }
      finally {
        process.exit(1);
      }
...
function compileDefinition(def) {
  ...
  try {
    ...
    postMessage({ error: 'Compilation error: ' + err });
