...
class ActorSystem {
    ...
    return P.resolve()
      ...
      .then(Definition0 => {
        ...
        switch (options.mode || 'in-memory') {
          ...
          case 'threaded':
            return this._createThreadedActor(Definition, parent, _.defaults({ name: actorName }, options));
   ...
   * @private
   */
  _createThreadedActor(definition, parent, options = {}) {
    // TODO
  }
  ...
  _createRemoteActor(definition, parent, options) {
    return new P((resolve, reject) => {
      ...
      socket.on('connect', common.guard(reject, () => {
        this.generateActorCreationMessage(definition, parent, _.defaults({ mode: 'remote' }, options))
          .then(createMsg => {
